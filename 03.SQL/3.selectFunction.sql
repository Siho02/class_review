...
select enmae, empno from emp where mod(empno, 2) = 1 ;
select enmae, empno from emp where mod(empno, 2) != 1 ;
...
-- upper() : 대문자[uppercase]
-- lower() : 소문자[lowercase]
select upper('abCDaef') from dual;
select lower('abCDaef') from dual;
...
select length('a'), lengthb('b'), length('가'), lengthb('가') from dual;
...
-- 대부분 프로그래밍 언어(python, sql, java script, java)
...
--8.? 2020년 2월의 마지막 날짜는?
select last_day('2020-02-12') from dual;
select last_day('20-02-12') from dual;
select last_day('2020/02/02') from dual;
select last_day('20/02/02') from dual; 
select last_day('2020.02.02') from dual;
select last_day('20.02.02') from dual;
select last_day('2020,02,02') from dual;
select last_day('20,02,02') from dual;
select last_day('20200202') from dual;
select last_day('200202') from dual;
...
select sysdate, round(sysdate), to_char(trunc(sysdate)), 'yyyy-mm-dd hh:mi:ss am' from dual;
...
--[2] to_date() : 날짜로 변경 시키는 함수
-- 단순 숫자로는 date 타입과 연산 불가
/* 모두 오류
select sydate - 20200719 from dual;
select sydate - '20200719' from dual;
select sydate - '2020/07/19' from dual;
*/

--1. 올해 며칠이 지났는지 검색(포멧 yyyy/mm/dd)
-- 단순 숫자를 날짜 포맷으로 변경
select round(sysdate - to_date(20200719)) as dayfrom_200719 from dual;
...
-- 오류 : select '20000' - '10000' from dual;
select to_number('20000') - to_number('10000') from dual;

-- 문자열을 숫자로 변환시 포맷도 설정 가능
...
select sal, decode(JOB, 'ANALYST', sal*1.05,
                        'SALESMAN', sal*1.1,
                        'MANAGER', sal*1.15, 
                        'CLERK', sal*1.2,
                        sal) as 인상분 
