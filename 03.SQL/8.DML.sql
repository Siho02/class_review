...
- DDL과 DML은 다름
	...
	- DDL(create, drop, alter)은 commit & rollback은 무효한 명령어
*/
...
1. insert sql문법
	1-1. 모든 칼럼에 데이터 저장시 
		...
		-- ex) insert into people values('정승목', 26);

	1-2.  특정 칼럼에만 데이터 저장시,
		명확하게 칼럼명 기술해야 할 경우 
		insert into table명 (칼럼명1, ...) values(칼럼과매핑될데이터1...)
		-- ex) insert into people (age) values(10)l
	...
	1-3. 하나의 sql문장으로 다수의 table에 데이터 입력 방법
		...
		/* ex)
			insert all
				into people values('김철수', 10)
				into poeple values('김미미', 15)
			select * from dual;x
		*/ 
/*	
2. update 
...
4. merge(병합)
	...
	- syntax
		merge into 병합받을테이블명 
		using 이미데이터를보유한 테이블명
		on 조건식(병합받을테이블.기준컬럼 = 데이터제공테이블.컬럼)
		when matched then
			참인 경우 실행(update)
...
-- *** insert ****
--1. 칼럼명 기술없이 데이터 입력
insert into people values('조태익', 29);


--2. 칼럼명 기술후 데이터 입력 
insert into people (name, age) values('김뫄뫄', 26);
insert into people (age, name) values(26, '박솨솨');
...
--? 같은 소회의실에 있던 팀원들 정보를 저장하기
-- empno deptno는 임시로 설정, ename은 팀원 이름으로 설정
rollback;

select deptno from emp01;

insert all
	into emp01 (empno, ename, deptno) values (11, '이진의', 13)
	into emp01 (empno, ename, deptno) values (13, '반재광', 42)
select * from dual;
select * from emp01;
...
update emp01 set detpno = 30, job = 'MANAGER' where ename = 'SMITH';
select * from emp01;
...
/*
produce01 과 produce02 : 새로운 제품들에 대한 관리 테이블
produce_total은 : 재고 관리 테이블

제품이 새로 생산 되는 경우 : produce01, produce02에 insert
produce_total은 새로운 데이터 유입시에는 insert
				이미 존재하는 데이터는 update를 통해 관리
...
commit;     --영구 저장 >> 이 직후에는 rollback 해도 insert된 데이터 삭제 불가
