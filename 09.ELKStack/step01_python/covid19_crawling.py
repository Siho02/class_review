...
def crawl_data():
    ...
    # 
    file_name = 'C:/ELKStack/0.dataset/covid19_ko_' + time.strftime('%Y-%m-%d', time.localtime(time.time())) + '.csv'
    df.to_csv(file_name, index=False, header=False, encoding='utf8')

schedule.every().day.at("12:11").do(crawl_data)

while True:
    schedule.run_pending()
    time.sleep(1) #재우는 시간 늘려도됨
