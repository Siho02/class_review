...
import json 
import collections ##데이터를 어떤 구조로 관리한 것이가를 의미하는 자료구조를 지원하는 library
...
class EmpDAO:
    ...
    # 모든 직원 검색해서 반환
    # 동기 방식으로 요청 <-> 응답
    # 모든 직원 정보를 응답 시의 구조는 보편적으로 선호하는 JSON 포맷
    # Rest API는 url의 요청 방식도 처리 로직 + url 이름도 처리 로직
    # Rest API - url 정보가 요청 처리 리소스 + JSON 포맷으로 응답 + 비동기
    # json 포맷으로 가공해서 반환
    # python에서 json     
    ...
    def empall(self):  
        ...
        try:
            ...
            try:
                ...
                rows = cur.fetchall() 
                #json 포맷으로 가공 : empno / ename / sal key - json배열
                #다양한 방법 : 저장하는 순서를 유지하는 구조의 dict 클래스
                v = []  #v 변수에 python 구조의 dict 구조 저장 >> data 변수를 json 포맷으로 변환
                for row in rows:
